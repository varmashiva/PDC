                    ...
                    <div className="mt-auto flex flex-wrap items-center justify-between gap-6 border-t border-white/10 pt-6">
                        <div className="flex gap-4 md:gap-8">
                            <div className="flex flex-col text-white">
                                <span className="text-[0.45rem] md:text-[0.55rem] font-bold text-white/30 tracking-widest uppercase mb-0.5 md:mb-1">TIME</span>
                                <span className="text-xs md:text-sm font-bold uppercase">{workshop.time}</span>
                            </div>
                            <div className="flex flex-col text-white">
                                <span className="text-[0.45rem] md:text-[0.55rem] font-bold text-white/30 tracking-widest uppercase mb-0.5 md:mb-1">AVAILABILITY</span>
                                <span className="text-xs md:text-sm font-bold text-[#ff1a1a] uppercase">[{workshop.seats} SLOTS]</span>
                            </div>
                        </div>
...
const WorkshopShowcase = () => {
    ...
    useEffect(() => {
        const fetchWorkshops = async () => {
            try {
                ...
                const now = new Date();
                const activeWorkshops = data.filter((w) => {
                ...
                setWorkshops(activeWorkshops.slice(0, 4));
            } catch (error) {
        ...
        const handleNewWorkshop = (newWS) => {
            setWorkshops(prev => [newWS, ...prev].slice(0, 4));
        };

        fetchWorkshops();
        socket.on('newWorkshop', handleNewWorkshop);
        return () => socket.off('newWorkshop', handleNewWorkshop);
    }, []);
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            // The horizontal scroll timeline incorporating best practices
    ...
    if (workshops.length === 0) {
        return (
            <div className="w-full h-80 bg-white rounded-2xl flex items-center justify-center font-['Outfit']">
                <h2 className="text-black/10 text-4xl font-black uppercase">NO SESSIONS</h2>
            </div>
        );
    }
        ...
        >
            ...
            >
                ...
                <div 
                    ...
                    style={{ paddingLeft: '2vw' }} // Start position for the first card
                >
                    {/* Persistent Side Info - Part of the scroll layer but stays "left" initially */}
                    <div className="flex-shrink-0 w-[90vw] sm:w-[85vw] md:w-[30vw] px-8 sm:px-12 flex flex-col justify-center z-20">
                        ...
                        <p className="text-black/60 text-sm md:text-sm max-w-xs leading-relaxed font-semibold">
                            JOIN OUR EXCLUSIVE SESSIONS TO MASTER THE FLOW. LIMITED SLOTS AVAILABLE.
                        </p>
                    ...
                    {/* End Spacer to allow final card to fully center */}
            ...
            <div className="absolute right-10 top-1/2 -translate-y-1/2 hidden lg:block">
                <span className="text-[0.5rem] font-bold text-black/10 tracking-[1em] uppercase -rotate-90 origin-center whitespace-nowrap">
                    FEUGO PREMIUM WORKSHOPS SERIES 2026
                </span>
            </div>
        </section>
    );
